import {userSignup,emailSignin,resetPassword  } from "../service/service";
import { NextRequest,NextResponse } from "next/server";
import { signupReturn, userTypes } from "@/app/type/dbTypes";
...
export const usersignup = async (req: NextRequest):Promise<signupReturn> => {
  ...
  if(!user) return {error:"all fields required"};

  try {
    const result = await userSignup(user);
    return {data:result}
  } catch (error) {
    return {error:`failed to create db, ${error}`};
  }
};
